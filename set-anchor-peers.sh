...
# ============================================================================
# SET ANCHOR PEERS FOR KYCTOKENCHANNEL
...
setAnchorPeer() {
    ...
    configtxlator proto_encode \
        ...
        --type common.Config \
        --output /tmp/config.pb 2>/dev/null

    configtxlator proto_encode \
        --input /tmp/modified_config.json \
        --type common.Config \
        --output /tmp/modified_config.pb 2>/dev/null
    ...
    configtxlator proto_decode \
        ...
        --output /tmp/config_update.json 2>/dev/null
    ...
    if peer channel update \
        -f /tmp/anchor_update.pb \
        -c "$CHANNEL" \
        -o localhost:7050 \
        --ordererTLSHostnameOverride orderer.example.com \
        --tls \
        --cafile "$ORDERER_CA" 2>/dev/null; then
...
echo "============================================================================"
echo "Setting Anchor Peers for kycTokenChannel"
echo "============================================================================"

echo -e "\n${YELLOW}kycTokenChannel${NC}"

setAnchorPeer "org1" "Org1MSP" 7051 "kycTokenChannel"
setAnchorPeer "org2" "Org2MSP" 8051 "kycTokenChannel"
setAnchorPeer "org3" "Org3MSP" 9051 "kycTokenChannel"
